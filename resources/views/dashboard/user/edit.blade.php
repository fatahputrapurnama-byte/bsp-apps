          ...
          <form method="post" action="{{ url('dashboard/user/'.$user->id) }}" enctype="multipart/form-data">
            ...
            @csrf
              ...
              <div class="form-group mb-md-0">
                  ...
                  <input type="radio" onchange="validasi()" id="is_admin_true" name="is_admin" value="1">
                  <label for="html">Ya</label><br>
                  <input type="radio" onchange="validasi()" id="is_admin_false" name="is_admin" value="0">
                  <label for="html">Bukan</label><br>
                  @if ($user->is_admin == 1)
                      <script>
                        var radioElement = document.getElementById('is_admin_true');
                        radioElement.checked = true;
                      </script>
                  @else
                      <script>
                        var radioElement = document.getElementById('is_admin_false');
                        radioElement.checked = true;
                      </script>
                  @endif
              </div>
